#include<iostream>
#include<set>
#include<stdint.h>
#include<fstream>
#define rotate_left(value, shift) ((value << shift) | (value >> (32 - shift)))

uint8_t Sbox[] = {
    
    };

uint32_t L(uint32_t x) {
    return x ^ rotate_left(x,2) ^ rotate_left(x,10) ^ rotate_left(x,18) ^ rotate_left(x,24);
}


uint32_t R(uint32_t x) {
    uint32_t y;
    for (int i = 0; i < 4; ++i)
        *((uint8_t*)(&y)+i) = Sbox[*((uint8_t*)(&x)+i)];
    return y;
}

uint32_t T(uint32_t x) {
    return L(R(x));
}

int main() {
    uint32_t a2 = 0xf3000030;
    std::set<uint32_t> DiffSet;
    for (uint32_t x = 0; x < 0xFFFFFFFF; ++x)
        DiffSet.insert(T(x)^T(x^a2));
    DiffSet.insert(T(0xFFFFFFFF)^T(0xFFFFFFFF^a2));
    std::ofstream outfile("DiffSetCPP.txt");
    for (uint32_t element : DiffSet)
        outfile << element << std::endl;
    outfile.close();
}